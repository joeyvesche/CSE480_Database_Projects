...
class Connection(object):
    ...
    def execute(self, statement):
        ...
        def create_table(tokens):
            ...
            if if_not_exists:
                self.database.create_new_table_if_not_exists(table_name, column_name_type_pairs, id(self))
            else:
                self.database.create_new_table(table_name, column_name_type_pairs, id(self))
        ...
        def drop_table(tokens):
            ...
            if tokens and tokens[0] == "IF":
                ...
                if_exists = True
            table_name = tokens.pop(0)
            self.database.drop_table(table_name, if_exists, id(self))
        ...
        def insert(tokens):
            ...
            self.database.insert_into(table_name, columns, row_values, id(self))
        ...
        def delete(tokens):
            ...
            self.database.delete(table_name, where_clause, id(self))
        ...
        def update(tokens):
            ...
            while True:
                ...
                pop_and_check(tokens, ",")
            where_clause = parse_where_clause(tokens) if tokens and tokens[0].upper() == "WHERE" else None
            self.database.update(table_name, set_columns, where_clause, id(self))
        ...
        assert tokens[0] in {"CREATE", "INSERT", "SELECT", "DELETE", "UPDATE", "SET", "DISTINCT", "DROP",
        ...
        if tokens[0].upper() == "CREATE":
            ...
            drop_table(tokens)
        elif tokens[0].upper() == "BEGIN":
            self.database.start_transaction(tokens[1] if len(tokens) > 1 else "DEFERRED", id(self))
            self.transaction_started = True
        elif tokens[0].upper() == "COMMIT":
            if self.transaction_started:
                self.database.commit_transaction(id(self))
                self.transaction_started = False
            else:
                raise Exception("COMMIT statement without an open transaction")
        elif tokens[0].upper() == "ROLLBACK":
            self.database.rollback_transaction(id(self))
...
class Database:
    ...
    def create_new_table(self, table_name, column_name_type_pairs, connection_id):
    ...
    def create_new_table_if_not_exists(self, table_name, column_name_type_pairs, connection_id):
        if self.in_transaction:
            if table_name not in self.tables[connection_id]:
                self.create_new_table(table_name, column_name_type_pairs, connection_id)
        else:
            if table_name not in self.tables:
                self.create_new_table(table_name, column_name_type_pairs, connection_id)

    def drop_table(self, table_name, if_exists, connection_id):
        if self.in_transaction:
            if table_name in self.connections[connection_id]:
                self.connections[connection_id].pop(table_name)
            elif not if_exists:
    ...
    def insert_into(self, table_name, columns, row_values, connection_id):
        if self.in_transaction:
            self.acquire_lock("RESERVED", table_name, connection_id)
            if connection_id not in self.transaction_connections:
                self.transaction_connections[connection_id] = []
            if "SHARED" in self.transaction_connections[connection_id]:
                self.transaction_connections[connection_id].remove("SHARED")
            self.transaction_connections[connection_id].append("RESERVED")

        if self.in_transaction:
            table = self.connections[connection_id][table_name]
        else:
    ...
    def select(self, output_columns, table1_name, order_by_columns=None, where_clause=None, distinct=None,
               join_type=None, table2_name=None, connection_id=0):
        if self.in_transaction:
            assert table1_name in self.connections[connection_id]
        else:
            assert table1_name in self.tables
        if self.in_transaction:
            self.acquire_lock("SHARED", table1_name, connection_id)
            if connection_id not in self.transaction_connections:
                self.transaction_connections[connection_id] = []
            if "RESERVED" in self.transaction_connections[connection_id]:
                self.transaction_connections[connection_id].remove("RESERVED")
            self.transaction_connections[connection_id].append("SHARED")
        if self.in_transaction:
            table1 = self.connections[connection_id][table1_name]
        else:
            table1 = self.tables[table1_name]
        if join_type:
            if join_type == "LEFT OUTER JOIN":
                if self.in_transaction:
                    assert table2_name in self.connections[connection_id]
                    table2 = self.connections[connection_id][table2_name]
                else:
    ...
    def delete(self, table_name, where_clause=None, connection_id=0):
        if self.in_transaction:
            self.acquire_lock("RESERVED", table_name, connection_id)
            if connection_id not in self.transaction_connections:
                self.transaction_connections[connection_id] = []
    ...
    def update(self, table_name, set_values, where_clause=None, connection_id=0):
        if self.in_transaction:
            assert table_name in self.connections[connection_id]
        else:
    ...
    def start_transaction(self, mode="DEFERRED", connection_id=0):
        self.in_transaction = True

        if len(self.connections) == 0:
            self.connections[connection_id] = copy.deepcopy(self.tables)
        else:
            for i in self.connections:
                self.connections[connection_id] = copy.deepcopy(self.connections[i])
                break
        ...
        if mode == "TRANSACTION":
            ...
            for i in self.connections[connection_id].keys():
                self.acquire_lock("RESERVED", i, connection_id)
            if connection_id not in self.transaction_connections:
                self.transaction_connections[connection_id] = []
            self.transaction_connections[connection_id].append("RESERVED")

        if mode.upper() == "EXCLUSIVE":
            if self.transaction_depth > 0 or self.locks:
                raise Exception("Exclusive transaction cannot be started while another transaction is in progress")
            self.in_transaction = True
            self.acquire_lock("EXCLUSIVE", None, connection_id)  # Pass None as table_name to initialize locks dictionary
        ...
        self.tables_backup = copy.deepcopy(self.connections[connection_id])
        self.transaction_tables[connection_id] = set()
        self.transaction_id = id(self.transaction_tables)

    def commit_transaction(self, connection_id=0):
        if len(self.transaction_connections[connection_id]) == 1:
            if len(self.transaction_connections) > 1:
                self.transaction_connections[connection_id] = "EXCLUSIVE"
                for i, j in self.transaction_connections.items():
                    if "EXCLUSIVE" in j or "SHARED" in j or "RESERVED" in j:
                        raise Exception("Cannot have Exclusive lock with other locks")

        if connection_id not in self.transaction_connections:
            raise Exception("Connection not in transaction")

        # Check if any locks are blocking the promotion to an exclusive lock
        blocking_tables = []
        for table_name, lock in self.locks.items():
            for j in lock:
                if j != 0 and table_name in self.transaction_tables[connection_id]:
                    blocking_tables.append(table_name)

                    if j == "SHARED":
                        raise Exception(
                            f"Cannot gain exclusive lock on table {table_name} because a shared lock is already held")
        if blocking_tables:
            raise Exception(
                f"Cannot promote to exclusive lock due to blocking locks on tables: {', '.join(blocking_tables)}")

        if self.transaction_depth == 1:
            # Promote all locks to exclusive locks
            for table_name in self.connections[connection_id].keys():
                self.acquire_lock("EXCLUSIVE", table_name, connection_id)
            ...
            # Decrement the transaction depth and clear the transaction tables for this connection
            self.transaction_depth -= 1
            self.transaction_tables[connection_id].clear()

    def acquire_lock(self, lock_type, table_name=None, connection_id=0):
        for i, j in self.transaction_connections.items():
            if j.count("RESERVED") >= 2 or (j.count("SHARED") >= 1 and j.count("EXCLUSIVE") >= 1):
                raise Exception("Cannot have multiple locks of this type")
        if table_name:
            if table_name not in self.locks:
                self.locks[table_name] = {}  # initialize table lock dictionary

            if lock_type == "SHARED":
                for lt in self.transaction_connections:
                    if lt == "EXCLUSIVE":
                        raise Exception(
                            f"Table {table_name} cannot be locked with a shared lock because an exclusive lock is already held")
                    elif lt == "SHARED":
                        raise Exception("Cannot have multiple locks of this type")

            self.locks[table_name][lock_type] = threading.Lock()
            self.locks[table_name][lock_type].acquire()

    def release_lock(self, table_name=None):
        if table_name is not None:
            if self.locks[table_name] == "RESERVED":
                if len(self.locks) > 1:
                    raise Exception
                else:
                    self.locks[table_name] = "EXCLUSIVE"

        if table_name is not None:
            self.locks.pop(table_name, None)
        else:
            self.locks = {}

    def rollback_transaction(self, connection_id=0):
        if connection_id not in self.transaction_connections:
            raise Exception("Connection not in transaction")

            # Reset tables to backup
        self.tables = copy.deepcopy(self.tables_backup)

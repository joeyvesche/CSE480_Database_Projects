...
class Connection(object):
    ...
    def executemany(self, statement, param):
        for parameter in param:
            self.execute(statement, parameter)
    ...
    def execute(self, statement, parameters=None):
        ...
        if parameters:
            for p in parameters:
                if isfloat(p):
                    ...
                    p = "'" + str(p) + "'"
                statement = statement.replace('?', p, 1)
        ...
        def create_table(tokens):
            ...
            while True:
                ...
                if tokens[0] == "DEFAULT":
                    ...
                    default[column_name] = default_value
                    self.default[table_name] = default

                assert column_type in {"TEXT", "INTEGER", "REAL"}
                column_name_type_triples.append((column_name, column_type, default_value))
                comma_or_close = tokens.pop(0)
                if comma_or_close == ")":
                    break
                assert comma_or_close == ','
            self.database.create_new_table(table_name, column_name_type_triples)
        ...
        def insert(tokens):
            ...
            if tokens[0] == "VALUES":
                ...
                pop_and_check(tokens, "DEFAULT")
                pop_and_check(tokens, "VALUES")
                for key, value in self.default[table_name].items():
                    row_contents = []
                    columns.append(key)
        ...
        def select(tokens):
            ...
            if tokens[0] in ["MAX", "MIN"]:
                agg_func = tokens.pop(0)
                pop_and_check(tokens, "(")
                output_col = tokens.pop(0)
                pop_and_check(tokens, ")")
        ...
        def create_view(tokens):
            """
            Determines the view name and SELECT statement from
            tokens and creates the view
            """
            pop_and_check(tokens, "CREATE")
            pop_and_check(tokens, "VIEW")
            view_name = tokens.pop(0)
            pop_and_check(tokens, "AS")
            select_statement = tokens
            self.views[view_name] = select_statement


        def parse_view_statement(view_statement):
            """
            Given a view statement, parse the SELECT statement and
            determine the tables it references
            """
            select_start = view_statement.upper().index("SELECT")
            view_name_start = view_statement.upper().index("FROM") + 4
            view_name_end = view_statement.upper().index("WHERE") if "WHERE" in view_statement.upper() else None
            if view_name_end is None:
                view_name_end = len(view_statement)
            view_name = view_statement[view_name_start:view_name_end].strip()
            return view_name
        ...
        if tokens[0].upper() == "CREATE":
            ...
            if tokens[3] in self.views:
                view_name = tokens[3]
                if view_name in self.views:
                    select_statement = copy.deepcopy(self.views[view_name])
                    return select(select_statement)

                else:
                    raise ValueError("View '{}' not found".format(view_name))
            else:
...
class Database:
    ...
    def get_table_columns_with_default(self, table_name):
        """
        Returns a dictionary with the column names as keys and the default values as values
        for the specified table.
        """
        return self.tables[table_name].get_columns_with_default()

    def create_new_table(self, table_name, column_name_type_triples):
        assert table_name not in self.tables
        self.tables[table_name] = Table(table_name, column_name_type_triples)
        return []
    ...
    def select(self, output_columns, table1_name, order_by_columns=None, where_clause=None, distinct=None,
               join_type=None, table2_name=None, min_max=None):
        ...
        if join_type:
            ...
            result = table1
        return result.select_rows(output_columns, order_by_columns, where_clause, distinct, min_max)
...
class Table:
    def __init__(self, name, column_name_type_triples):
        self.name = name
        self.column_names, self.column_types, self.default_values = zip(
            *[(name, type_, None) if len(triple) < 3 else triple for name, type_, *triple in column_name_type_triples]
        )
        self.rows = []

    def get_columns_with_default(self):
        return list(zip(self.column_names, self.column_types, self.default_values))

    def insert_new_row(self, row_contents):
        assert len(self.column_names) == len(row_contents)
        row = {}
        for i, col_name in enumerate(self.column_names):
            if row_contents[i] is not None:
                row[col_name] = row_contents[i]
            else:
                row[col_name] = self.default_values[i]
    ...
    def select_rows(self, output_columns, order_by_columns, where_clause=None, distinct=None, min_max=None):
        ...
        def check_columns_exist(columns):
            ...
            if "DESC" in column_names:
                return
            assert all(col in self.column_names for col in
                       column_names), f"Columns not found: {', '.join(set(column_names) - set(self.column_names))}"
        ...
        def sort_rows(rows, order_by_columns):
            ...
            for col in reversed(order_by_columns):
                reverse = False
                if col.endswith("DESC"):
                    reverse = True
                    col = col[:-4].strip()
                    col = order_by_columns[0]
                rows = sorted(rows, key=lambda row: get_value(row, col), reverse=reverse)
                if reverse:
                    break
            return rows
        ...
        def aggregate_rows(rows, output_columns, min_max):
            if min_max == "MAX":
                return [rows[-1]]
            else:
                return [rows[0]]
        ...
        if min_max is not None:
            aggregated_rows = aggregate_rows(sorted_rows, expanded_output_columns, min_max)
            return generate_tuples(aggregated_rows, output_columns)
